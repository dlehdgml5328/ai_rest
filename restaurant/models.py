...
class Article(models.Model):
    ...
    show_at_index = models.BooleanField(default=False) # 기본값은 False (즉, 체크 안 된 상태로 저장됨).
    is_published = models.BooleanField(default=False) # 기본값은 False (즉, 체크 안 된 상태로 저장됨).
    created_at = models.DateTimeField("생성일", auto_now_add=True) # 객체가 처음 생성될 때만 현재 시각으로 자동 저장.
    modified_at = models.DateTimeField(auto_now=True) # 객체가 수정될 때마다 자동으로 시간 업데이트됨.
...
class Restaurant(models.Model):
    ...
    rating = models.DecimalField("평점", max_digits=3, decimal_places=2, default="0.0") # 예: 9.99까지 가능 (최대 소수점 둘째 자리까지 표현됨).
    rating_count = models.PositiveIntegerField("평가수", default=0) # 0 이상의 양의 정수만 허용하는 필드 (예:좋아요 수, 평가 수 등).
    start_time = models.TimeField("영업 시작 시간", null=True, blank=True) # 시/분/초 단위의 시간만 저장하는 필드 (날짜는 없음). 예: 18:30:00
    ...
    category = models.ForeignKey(
        "RestaurantCategory", on_delete=models.SET_NULL, blank=True, null=True # 참조된 카테고리 삭제 시, 이 필드 값을 NULL로 설정 (데이터 보존).
    )
    tags = models.ManyToManyField("Tag", blank=True) # 다대다(M:N) 관계 설정. 하나의 레스토랑이 여러 태그를 가질 수 있음.
...
class RestaurantImage(models.Model):
    ...
    is_representative = models.BooleanField("대표 이미지 여부", default=False) # 체크하면 대표이미지로 설정됨, 기본값은 대표이미지 가 체크되지 않아서 이미지 저장후 체크여부 선택   
    order = models.PositiveIntegerField("순서", null=True, blank=True)
    name = models.CharField("이름", max_length=100, null=True, blank=True)
    image = models.ImageField("이미지", max_length=100, upload_to="restaurant") # 사용자가 이미지를 업로드하면 MEDIA_ROOT/restaurant/ 폴더 아래에 이미지 파일이 저장됩니다. MEDIA_ROOT가 /media/라고 가정하면
    created_at = models.DateTimeField("생성일", auto_now_add=True, db_index=True) # 데이터를 등록하면 생성일은 자동으로 생성됨
    ...
    def clean(self):
        ...
        if self.is_representative and images.exclude(id=self.id).count() > 0: # 대표 이미지를 2개 이상 지정하지 못하도록 막는 코드
...
class RestaurantMenu(models.Model):
    ...
    created_at = models.DateTimeField("생성일", auto_now_add=True, db_index=True) # 시간정렬을 할수도 있으니 적용하는것이 좋음 
    updated_at = models.DateTimeField("수정일", auto_now=True, db_index=True)
...
class Tag(models.Model):
    name = models.CharField("이름", max_length=100, unique=True) # 이 필드는 중복을 허용하지 않음
